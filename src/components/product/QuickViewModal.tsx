...
export const QuickViewModal: React.FC = () => {
  ...
  return (
    <AnimatePresence>
      {quickViewProduct && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 md:p-10 overflow-y-auto" role="dialog" aria-modal="true" aria-label={`Quick View: ${quickViewProduct.name}`}>
          {/* Backdrop */}
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          ...
          >
            ...
            <button
              ...
              className="absolute top-4 right-4 z-20 p-2 rounded-full bg-[#181A1F] text-[#8E929E] hover:text-[#F5F5F7] border border-white/10 transition-colors cursor-pointer"
            >
              <X className="w-5 h-5" />
            </button>
            ...
            <div className="w-full md:w-1/2 p-6 md:p-8 flex flex-col justify-between overflow-y-auto">
              <div>
                {/* Brand and SKU */}
                <div className="flex items-center justify-between text-xs uppercase tracking-widest text-[#CBD0DC] mb-2">
                ...
                <div className="mt-3 flex items-baseline gap-3">
                  {quickViewProduct.salePrice ? (
                    <>
                      ...
                      <span className="text-sm text-[#8E929E] line-through">
                ...
                {/* Short Description */}
                <p className="mt-4 text-sm text-[#CBD0DC] leading-relaxed">
                ...
                {quickViewProduct.variants && quickViewProduct.variants.length > 0 && (
                  <div className="mt-5 space-y-3">
                    {quickViewProduct.variants.map((v) => (
                      <div key={v.name}>
                        <span className="text-xs uppercase tracking-wider text-[#CBD0DC] font-medium block mb-2">
                          {v.name}
                        </span>
                          ...
                          {v.options.map((opt) => {
                            ...
                            return (
                              <button
                                ...
                                className={`px-3 py-1.5 text-xs rounded-lg border transition-all cursor-pointer ${isSelected
                                  ? 'border-[#D4AF37] bg-[#D4AF37]/10 text-[#F5F5F7]'
                                  : 'border-[#262930] text-[#CBD0DC] hover:border-white/20'
                                  }`}
                              >
                                {opt}
                              </button>
                ...
                <div className="mt-6 flex items-center gap-4">
                  ...
                  <div className="flex items-center rounded-lg border border-[#262930] bg-[#181A1F]">
                    <button
                      ...
                      className="px-3 py-1.5 text-sm text-[#CBD0DC] hover:text-[#D4AF37] transition-colors cursor-pointer"
                    >
                      -
                    </button>
              ...
              <div className="mt-8 space-y-3 pt-6 border-t border-[#262930]">
                <div className="flex gap-3">
                  <button
                    ...
                    className="flex-1 py-3 px-4 rounded-xl bg-[#181A1F] hover:bg-[#22252C] text-sm uppercase tracking-wider font-semibold text-[#F5F5F7] border border-[#262930] hover:border-[#D4AF37]/40 transition-all cursor-pointer"
                  >
                    Add to Shopping Bag
                  </button>
                  <button
                    ...
                    className={`p-3 rounded-xl border transition-all cursor-pointer ${inWishlist
                      ? 'bg-[#D4AF37] text-[#0B0C0E] border-[#D4AF37]'
                      : 'bg-[#181A1F] text-[#E8E8EC] border-[#262930] hover:text-[#D4AF37]'
                      }`}
                    aria-label={inWishlist ? `Remove ${quickViewProduct.name} from wishlist` : `Add ${quickViewProduct.name} to wishlist`}
                  >
                ...
                <button
                  ...
                  className="w-full py-3 px-4 rounded-xl bg-gradient-to-r from-[#D4AF37] to-[#C5A059] hover:from-[#E5C378] hover:to-[#D4AF37] text-[#0B0C0E] text-sm uppercase tracking-wider font-bold shadow-lg transition-all cursor-pointer"
                >
                  Instant Checkout
                </button>
                ...
                <button
                  ...
                  className="w-full text-center text-xs text-[#CBD0DC] hover:text-[#D4AF37] flex items-center justify-center gap-1.5 pt-2 transition-colors cursor-pointer"
                >
